...
#define AX_MODULE_NAME "axperiago_lulzactiveq"
...
unsigned int stored_max_speedd = 1000000;
unsigned int sleep_max_freqq = 200000;

static void lulzactive_early_suspend(struct early_suspend *handler) {
	struct cpufreq_lulzactive_cpuinfo *dbs_info;
	early_suspended = 1;
	dbs_info = &per_cpu(cpuinfo, 0);
	stored_max_speedd = dbs_info->policy->max;
	cpufreq_update_freq(0, dbs_info->policy->min, sleep_max_freqq);
        __cpufreq_driver_target(dbs_info->policy, sleep_max_freqq, CPUFREQ_RELATION_H);
	stop_rq_work();
}
...
static void lulzactive_late_resume(struct early_suspend *handler) {
	...
	early_suspended = 0;
	dbs_info = &per_cpu(cpuinfo, 0);
	cpufreq_update_freq(0, dbs_info->policy->min, stored_max_speedd);
