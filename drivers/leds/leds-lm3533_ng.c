...
static int autosuspend_delay_ms = 50;
...
static int lm3533_write(struct lm3533_data *lm, u8 val, u8 reg)
{
	...
	dev_vdbg(&lm->i2c->dev, "%s: 0x%02x to 0x%02x\n", __func__, val, reg);
		...
		const char *buf, size_t count)
{
	...
	rc = strict_strtoul(buf, 10, &als_current);
		...
		const char *buf, size_t count)
{
	...
	rc = !strict_strtoul(buf, 10, &reset) && reset == 1 ?
