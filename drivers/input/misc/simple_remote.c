...
#define DETECTION_DELAY 20
#define DETECTION_DELAY_FINAL 200
...
#define MIN_NUM_BTN_DECTECTIONS 3

#define DETECT_WORK_DELAY				\
	(jiffies + msecs_to_jiffies(DETECTION_DELAY))

#define DETECT_WORK_DELAY_FINAL				\
	(jiffies + msecs_to_jiffies(DETECTION_DELAY_FINAL))
...
static void simple_remote_plug_det_work(struct work_struct *work)
{
	...
	if (DETECTION_CYCLES > atomic_read(&jack->detect_cycle) &&
		...
		if (jack->current_accessory_state == DEVICE_HEADPHONE)
			mod_timer(&jack->plug_det_timer,
						DETECT_WORK_DELAY_FINAL);
		else
			mod_timer(&jack->plug_det_timer, DETECT_WORK_DELAY);
...
static void simple_remote_btn_det_work(struct work_struct *work)
{
	...
	for (i = 0; i < MIN_NUM_BTN_DECTECTIONS; i++) {
		jack->interface->read_hs_adc(&adc_value);
		button = simple_remote_attrs_parse_button_value(jack,
								adc_value);
		if (0 == i) {
			last_button = button;
			dev_dbg(jack->dev, "First call. Setting button\n");
		} else if (last_button != button) {
			dev_dbg(jack->dev, "Invalid button response.\n");
			goto done;
		}
		dev_dbg(jack->dev, "%s - Button ADC Value = %u\n", __func__,
			 adc_value);
		msleep(10);
	}
