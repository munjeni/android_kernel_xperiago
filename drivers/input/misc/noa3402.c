...
#define PS_CONTROL_ONE_SHOT		(1 << 0)
...
#define MAX_PS_ONE_SHOT_TIME	30	/* ms */
...
static void noa3402_init_register(struct noa3402_chip *chip)
{
	...
	chip->regs[INT_CONFIG] = INT_ALWAYS_UPDATE;	/* Active low */
			      ...
			      struct noa3402_chip *chip)
{
	...
	if (current_status != !!chip->active)
		ret = pdata->pwr_enable(chip, !!chip->active);
...
static int noa3402_device_open(struct input_dev *dev)
{
	...
	/* We used to start PS_CONTROL_REPEAT here, but when using the sysfs
	   prox interface (PS_CONTROL_ONE_SHOT) the chip sometimes stops
	   responding. Moving PS_CONTROL_REPEAT to the end of the prox sysfs
	   interface solved that problem. */
...
static ssize_t noa3402_prox_show(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	struct noa3402_chip *chip = dev_get_drvdata(dev);
	...
	if (chip->active & PS_POWER) {
		ret = i2c_smbus_write_byte_data(chip->client,
				regmap[PS_CONTROL], PS_CONTROL_ONE_SHOT);
		if (ret)
			goto exit;
		mdelay(1);
		total_delay++;
		ret = i2c_smbus_read_byte_data(chip->client,
							regmap[PS_CONTROL]);
		while (total_delay < MAX_PS_ONE_SHOT_TIME && ret) {
			dev_dbg(&chip->client->dev, "%s: ret = 0x%x\n",
								__func__, ret);
			mdelay(1);
			total_delay++;
			ret = i2c_smbus_read_byte_data(chip->client,
		...
		if (!ret) {
			gpio_state = gpio_get_value(pdata->gpio);
			prox_state = (chip->regs[INT_CONFIG] & INT_ACTIVE_HIGH)
						? !gpio_state : gpio_state;
			dev_dbg(&chip->client->dev, "%s: prox_state = %d\n",
							__func__, prox_state);
			ret = snprintf(buf, PAGE_SIZE, "%.1hhd\n", prox_state);
		} else {
			ret = -EIO;
		}
		if (i2c_smbus_write_byte_data(chip->client, regmap[PS_CONTROL],
							PS_CONTROL_REPEAT)) {
			dev_err(&chip->client->dev,
				"%s: Failed to enable PS_CONTROL_REPEAT\n",
				__func__);
			chip->regs[PS_CONTROL] = PS_CONTROL_OFF;
		} else {
...
static struct device_attribute attributes[] = {
	...
	__ATTR(prox, 0444, noa3402_prox_show, NULL),
				   ...
				   const struct i2c_device_id *id)
{
	...
	if (!pdata || !pdata->hw_setup || !pdata->pwr_enable) {
	...
	ret = pdata->hw_setup(chip, true);
	if (ret) {
		dev_err(&client->dev, "%s: hw_setup failed\n", __func__);
		goto exit_free_data;
	}

	ret = pdata->pwr_enable(chip, true);
	if (ret) {
		dev_err(&client->dev, "%s: pwr_enable failed\n", __func__);
		goto exit_hw_release;
	}
...
exit_power_off:
	(void)pdata->pwr_enable(chip, false);
exit_hw_release:
	(void)pdata->hw_setup(chip, false);
exit_free_data:
	kfree(chip);
exit:
...
static int noa3402_remove(struct i2c_client *client)
{
	...
	free_irq(chip->irq, chip);
	input_unregister_device(chip->input_dev);
	pdata->pwr_enable(chip, false);
	pdata->hw_setup(chip, false);
	i2c_set_clientdata(client, NULL);
	kfree(chip);
	return 0;
}
