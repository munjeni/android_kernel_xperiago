...
#define RMI4_IS_VALID_SLEEPMODE(mode) \
	(mode >= RMI4_SLEEP_MODE_NORMAL && mode <= RMI4_SLEEP_MODE_RESERVED1)
				      ...
				      const char *buf, size_t count)
{
	...
	err = strict_strtoul(buf, 10, &new_value);
	if (err < 0 || new_value < 0 || new_value > 1) {
					   ...
					   size_t count)
{
	...
	err = strict_strtoul(buf, 10, &new_value);
	if (0 > err || new_value < 0 || new_value > 1) {
...
static void irq_handler(int irq_id, void *data)
{
	...
	if (ddata->status.unconfigured) {
		...
		rmi4_f01_update_control(fdev, true);
...
static int rmi4_f01_start(struct rmi4_function_device *fdev)
{
	...
	if (rmi4_bus_request_irq(fdev, fdev, irq_handler)) {
