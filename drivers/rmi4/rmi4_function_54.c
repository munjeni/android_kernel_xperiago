...
#ifdef CONFIG_SEMC_GENERIC_RMI4_F54_WATCHDOG
...
#ifdef CONFIG_SEMC_GENERIC_RMI4_F54_DBG_HUMAN_READABLE
static void rmi4_f54_report_hr_data(struct rmi4_function_device *fdev)
{
	...
	int size = (ddata->query.number_of_transmitter_electrodes *
		    ddata->query.number_of_receiver_electrodes);
	u16 *data = (u16 *)ddata->report_data;
	/* Add space for data and additional chars (EOF\n + \0) */
	char buf[size  + ddata->query.number_of_transmitter_electrodes + 6];
	...
	dev_dbg(&fdev->dev, "Num TX electrodes: %d\n",
		 ddata->query.number_of_transmitter_electrodes);
	dev_dbg(&fdev->dev, "Num RX electrodes: %d\n",
		 ddata->query.number_of_receiver_electrodes);

	for (i = 0; i < ddata->query.number_of_transmitter_electrodes &&
		     count < sizeof(buf); i++) {
		/* The ranges used below are probably too low or at least
		 * incorrect. Needs update */
		for (j = 0; j < ddata->query.number_of_receiver_electrodes &&
			     count < sizeof(buf); j++) {
...
{
#ifdef CONFIG_SEMC_GENERIC_RMI4_F54_DBG_RAW_HEX
	int i;
	u16 tmp;
	struct rmi4_f54_drvdata *ddata = dev_get_drvdata(&fdev->dev);
	u16 *data = (u16 *)ddata->report_data;
...
static int rmi4_f54_start(struct rmi4_function_device *fdev)
{
	...
	err = rmi4_bus_request_irq(fdev, fdev, rmi4_f54_irqhandler);
	if (err) {
...
{
	pr_info("%s - Registering function %s\n", __func__, rmi4_f54.drv.name);
	return rmi4_bus_register_function_driver(&rmi4_f54);
}

static void __devexit rmi4_f54_exit(void)
{
	pr_info("%s - Unregistering function %s\n", __func__,
		rmi4_f54.drv.name);
