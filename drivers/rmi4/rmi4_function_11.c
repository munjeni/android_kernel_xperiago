				...
				struct f11_2d_sensor_query *squery)
{
	...
	dev_dbg(&fdev->dev, "Device supports:\n");
	dev_dbg(&fdev->dev, "Num sensors: %d\n", dquery->nbr_of_sensors);
	dev_dbg(&fdev->dev, "has query 9: %s\n",
		 dquery->has_query9 ? "Yes" : "No");
	dev_dbg(&fdev->dev, "has query 11: %s\n",
		 dquery->has_query11 ? "Yes" : "No");

	dev_dbg(&fdev->dev, "Sensor supports:\n");
	dev_dbg(&fdev->dev, "Number of fingers: %d\n",
		 squery->number_of_fingers);
	dev_dbg(&fdev->dev, "Has rel: %s\n", squery->has_rel ? "Yes" : "No");
	dev_dbg(&fdev->dev, "Has abs: %s\n", squery->has_abs ? "Yes" : "No");
	dev_dbg(&fdev->dev, "Has gestures: %s\n",
		 squery->has_gestures ? "Yes" : "No");
	dev_dbg(&fdev->dev, "Sensitivity adjust: %s\n",
		 squery->has_sensitivity_adjust ? "Yes" : "No");
	dev_dbg(&fdev->dev, "Configurable: %s\n",
		 squery->configurable ? "Yes" : "No");
	dev_dbg(&fdev->dev, "Num X electrodes: %d\n",
		 squery->num_of_x_electrodes);
	dev_dbg(&fdev->dev, "Num Y electrodes: %d\n",
		 squery->num_of_y_electrodes);
	dev_dbg(&fdev->dev, "Max electrodes: %d\n",
...
static int rmi4_f11_start(struct rmi4_function_device *fdev)
{
	...
	err = rmi4_bus_request_irq(fdev, fdev, rmi4_f11_irq_handler);
	if (err) {
...
{
	pr_info("%s - Registering device function %s\n", __func__,
		rmi4_f11.drv.name);
